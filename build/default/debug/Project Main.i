...
void Move_Cursor_L(unsigned char);
void Move_Cursor_R(unsigned char);
...
void PrintLINE1(unsigned char,unsigned char,unsigned char *);
...
void Move_Cursor_R(unsigned char NumShiftingR){
    ...
    _delay((unsigned long)((60)*(16000000UL/4000.0)));
    ...
    send_a_command(0x0F);
    _delay((unsigned long)((80)*(16000000UL/4000.0)));
}
...
void LCD_Start(void){
    unsigned char a[16] = "Welcome!       ." ;
    unsigned char b[16] = "Microwave  P1.3M" ;
    unsigned char i =0;
    send_a_command(0x01);
    send_a_command(0x0E);
    _delay((unsigned long)((10)*(16000000UL/4000.0)));
...
void lcd_init(void)
{
 ...
 send_a_command(0x38);
    send_a_command(0x80);
    _delay((unsigned long)((3)*(16000000UL/4000.0)));
}
...
void SETHEAT_LCD(void){
    unsigned char a[16] = "  <<SET HEAT>>  " ;
    unsigned char b[7] = "Pot.P1)" ;
    unsigned char i =0;
    send_a_command(0x01);
    send_a_command(0x0E);
    _delay((unsigned long)((10)*(16000000UL/4000.0)));
...
void ADC_LCD(unsigned char * ptrTemp){
    unsigned char i=0;
    static unsigned char firsttime=2;
    ...
    TRISE1 &=0;
    TRISE2 &=0;
        _delay((unsigned long)((70)*(16000000UL/4000.0)));
    }
    send_a_command(0xC0);
    send_a_command(0x0C);
...
void PrintLINE2(unsigned char place,unsigned char length,unsigned char * ptrSTR){
    TRISD &=0x00;
    TRISE1 &=0;
    TRISE2 &=0;
    _delay((unsigned long)((70)*(16000000UL/4000.0)));
    send_a_command(0xC0+place);
    send_a_command(0x0C);
    for (; ((*(ptrSTR+place))!='\0')&&(place<length);place++){
...
void ADC_Read(unsigned char *Temp){
  ...
  ADC_Var= ((ADC_Var *( (double) 300/1023))+50);
  Temp[0]=((ADC_Var%1000)/100)+0x30;
  Temp[1]=((ADC_Var%100)/10)+0x30;
  Temp[2]=(ADC_Var%10)+0x30;
  Temp[3]=0b11011110;
  Temp[4]='C';
...
unsigned char TMR1Counter=0;
unsigned char Temp[5];
unsigned char digit=0;
unsigned char HeatSET=0;
unsigned char TIME[5]={"00:00"};
static unsigned int TIMEinSec=0;
extern unsigned char Key;
volatile unsigned char STATEF=0;

void __attribute__((picinterrupt(("")))) ISR_H();
